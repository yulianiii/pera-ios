...
struct OptInAssetListItem: Hashable {
    ...
    let model: AssetDecoration
    let viewModel: OptInAssetListItemViewModel

    init(asset: AssetDecoration) {
        self.model = asset
        self.viewModel = OptInAssetListItemViewModel(asset: asset)
    }
    ...
    static func == (
        lhs: OptInAssetListItem,
        rhs: OptInAssetListItem
    ) -> Bool {
        return
